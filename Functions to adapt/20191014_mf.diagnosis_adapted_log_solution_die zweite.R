mf.diagnosis.logsol.integrated=function(x,ET_Main,TL_out,fleet,n.fleet,B.Input,Beta,TopD,FormD,TLpred,n.TL,range.TLpred,lim.high.TL,range.highTL,Fish_mort, Fish_mort_acc){
  ...
  # 2.) CALCULATING PRODUCTION ----
   ...
   ## C.) CALCULATING ACCESSIBLE PRODUCTION ----
    ...
    # if (is.nan(ET_Main[1, "flow_acc"]==0)){
      cas <- 1
    ...
    # }
      ...
      # else {
      #   cas <- 2
      # # If trophic level 1 is accessible, I can directly adjust the accessible
      # # production,flow and mean_flow at trophic level 1 given the changes in
      # # the biomass input control
      # 
      # # Accessible flow : Gascuel et al. 2011; Equation A4 (The general flow eq.)
      # x[['flow_acc']][1] <- ET_Main[1, "flow_acc"] * x[['flow']][1]/ET_Main[1, "flow"]
      # # mean accessible flow : Gascuel et al. 2011; Equation A6 (Integrated version of the flow eq.)
      # x[['flow_mean_acc']][1] <- ET_Main[1, "flow_mean_acc"] * x[['flow_mean']][1]/ET_Main[1, "flow_mean"]
      # # production : Gascuel et al., 2011; Equation A9 (Production at a given TL)
      # x[['Prod_MF_acc']][1] <- ET_Main[1, "P_acc"] * x[['Prod_MF']][1]/ET_Main[1, "P"]
      # 
      # # Now I again recalculate the accessible flow at trophic level 2 (Equation A4)
      # 
      # x[["flow_acc"]][2]<- x[["flow_acc"]][1]*exp(-(ET_Main[(1), "N_loss_acc"]+
      #                       (-log(1-x[["Fish_mort_acc"]][1]/x[['Kin_MF_acc']][1])))*
      #                     (TL_out[2] - TL_out[1]))
      # }
  ...
  # 4.) CALCULATING KINETICS FROM TOP-DOWN EQUATION (Gascuel et al., 2011; Equation A13)----
   ## A.) FOR TL=1----
    ...
    if (cas==2){   # iF trophic level 1 is fished, than we have accessible biomass and kinetics
      x[['Kin_MF_acc']][1] <- (ET_Main[1, "Kin_acc"] - ET_Main[1, "Fish_mort_acc"])* 
                              (1 + TopD[1]*
                              (sum(x[['BIOM_MF_acc']][TL_out[TL_out>=2&TL_out<=2.3]])^FormD[1]- 
                              sum(ET_Main[TL_out[TL_out>=2&TL_out<=2.3], "B_acc"])^FormD[1])/
                              (sum(ET_Main[TL_out[TL_out>=2&TL_out<=2.3], "B_acc"])^FormD[1]))+  
                              x[["Fish_mort_acc"]][1]
    }
  ...
  # 5.) CALCULATING THE DIFFERENCE BETWEEN TEMPORAL PRODUCTION AND KINETICS AND----
    ...
    # x[['SC2']] <- round(sum((x[['Prod_MF']] - x[['Prod_MF_TMP']])^2), digits=12)
    x[['SC2']] <- round((sum(x[['Prod_MF']]) - sum(x[['Prod_MF_TMP']])) * 1E3)
    # x[['SC']] <- round(sum((x[["Kin_MF"]] - x[["TEMP_Kin"]])^2), digits=12)
  ...
  if (cas==1){Catches <-x[['BIOM_MF_acc']][-1]*x[['Fish_mort_acc']][-1]}
  # names(Catches)<-seq(2,7,0.1)
  if (cas==2){Catches <-x[['BIOM_MF_acc']]*x[['Fish_mort_acc']]}
  ...
  if(cas==2){
    for(i in 1:n.fleet){
      x[['Catches']][[fleet[i]]]=Catches*x[['mf']][[i]]* Fish_mort[[i]]/x[['Fish_mort']]
      names(x[['Catches']][[fleet[i]]])=TL_out
    }}
